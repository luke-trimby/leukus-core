...
export class PreloaderService extends AbstractService {
  ...
  public addAssetBundle(...bundles: ResolverBundle[]): void {
    ...
    bundles.forEach((bundle: ResolverBundle) => this.assetManifest.bundles.push(bundle));
  }
  ...
  protected async loadNextStage(): Promise<void> {
    ...
    Assets.addBundle(this.currentAssetBundle.name, this.currentAssetBundle.assets);
    Assets.load(this.currentAssetBundle.name,
      (progress: number) => this.onProgress(progress))
        .then(() => this.onComplete());
  }

  protected onProgress(progress: number): void {
    Log.i(`[PreloaderService] onProgress`, progress);
    this.onLoadingProgress.dispatch(progress, this.totalBundles - this.assetManifest.bundles.length, this.totalBundles);
  }

  protected onComplete(): void {
    Log.i(`[PreloaderService] Loading phase complete`);
    this.onStageLoaded.dispatch(this.currentAssetBundle.name, this.totalBundles - this.assetManifest.bundles.length, this.totalBundles);
